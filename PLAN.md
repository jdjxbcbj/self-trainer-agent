...
> ⚠️ 评分口径的开放决策：当前 GSB 是**关键词/正则匹配**，用户反馈「本地规则库非常智障，但接 LLM 也没好多少」。**方向是优先做「更高质量的本地规则库」**（更丰富的特征、场景 criteria 加权、红线精确化），LLM 兜底是否引入后续再定。评分权暂不交给模型。
...
- **隐私**：本地单机 SQLite，不外传；未来上服务端再考虑脱敏/删除策略。
...
1. **评分口径**：GSB 规则库（当前关键词匹配「很智障」）vs LLM 评分（「也没好多少」）——建议**先做更高质量的本地规则库**（更丰富特征 + 场景 criteria 加权 + 红线精确化），LLM 兜底后续再评估。评分权暂不交给模型。是否认可？
2. **是否按「Wave 0 → Wave 1 并行 → Wave 2 集成」推进？**（我建议是）
3. **并行子 Agent 用哪种方式**：默认「并行 `Agent` 工具调用（6 个同时，文件不相交）」；若要更强流程控制可用 `Workflow` 编排（需你明确一句「用 workflow」）。
