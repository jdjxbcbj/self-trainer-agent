...
| NPC 台词层级 | `≤25 yield`（优先）→ `26~44 low` → `45~69 mid` → `≥70 high` | `yield` 优先级最高，命中即服软，不与 `low` 重叠 |
| 通关 resolve（优秀） | 累计暴击数 ≥ `critsToPass` 且 对峙值 ≤ 40 | `critsToPass` 来自 SDB |
| 通关 resolve（及格） | 对峙值 ≤ 0 且暴击数 < `critsToPass` | 压平但暴击没凑够 → 判及格、`achievement_score` 降档；含「打过 1 次暴击但对峙值先到 0」，复盘文案点出「还差 N 次暴击即优秀」，不设半档 |
| 失控 deadlock | 对峙值 ≥ 85，或命中 `r-violence`（暴力红线） | |
| 到时 end | 达到 `round_limit` 仍未 resolve/deadlock → 触发复盘 | `round_limit = 20`（默认，冻结）；按当前对峙值判达成度 |
...
> ⚠️ **前后端数值分叉（已知事项）**：前端 demo 的判定参数比本节整体更宽松（更易暴击、更难死局、回合更短）。**后端以本节为准，前端仅作数据参考**。下表逐项对照（前端值以 `safety-confrontation-game/demo/app.js` 实测为准），联调时按表统一到后端口径、不必现场翻代码：

> | 参数 | 前端 demo（app.js） | 后端 §3.4 | 分叉方向 |
> |---|---|---|---|
> | 暴击线 | `score≥72 且命中维度≥5` | `score≥85 且无红线` | 前端更易暴击 |
> | 红线一票否决分 | `score=0` | `score=30`（RED_LINE_CAP） | 前端更严 |
> | 分数计算 | `min(18, 文本长度/4) 长度基础分 + Σ权重` | 纯 `Σ权重`（无长度分） | 前端话多更赚 |
> | 对峙值涨落·红线 | `+22` | `+25` | 接近 |
> | 对峙值涨落·暴击 | `−30` | `−15` | 前端降更快 |
> | 对峙值涨落·差回应 | `+8`（命中负向维度）/ `−6`（无负向） | `+10`（score<40）/ `−5`（40≤score<85） | 分档依据不同（前端按 tier、后端按 score 段） |
> | deadlock 线 | `对峙值≥95` | `对峙值≥85` | 前端更难死局 |
> | round_limit | `9` | `20` | 前端回合更短 |
> | 红线终局 | 任一红线 → 立即结束 | 仅 `r-violence` 直接失控，其余红线 `+25` 走对峙值 | 前端更快终止 |
> | 通关条件 | `暴击≥critsToPass 且无违规` 即通过 | 优秀需 `暴击≥critsToPass 且对峙值≤40`；另有「及格」`对峙值≤0` | 前端无对峙值门槛、无及格档 |

> 注意：本节参数只经**推演验证**（数学上不崩、终局可达、无震荡），**未经实际游玩**（前端 demo 参数至少跑过 demo）；联调时统一到后端口径，并按实际体验校准，**不排除修改本节参数**（届时同步 §9 决策第 4 项）。
...
6. **并行子 Agent 方式**：**并行 `Agent` 工具调用**（6 个文件不相交）；不用 Workflow（当前规模 Workflow 是过度设计，需要时再切）。
7. **`profile.py` 去留**：**删**——画像并入 `storage.py` 的 users 表，避免两处 DB 连接。
8. **`score()` 向后兼容**：**保留**一个兼容方法（`main.py`）。
9. **训练身份分流**：本期**只做场景隔离**（minorSafe 过滤场景），评分口径未成年/成人共用；分流后置。
